...
class IntesaFavorites(object):
    ...
    def __init__(self, auth):
        ...
        self.intesa_cursor = tweepy.Cursor(self.api.favorites, screen_name='intesasanpaolo')
    ...
    def get_favorites(self):
        while True:
            try:
                ...
                print "[LOG %s] Timeout reached.. I'm going to sleep for 5 minutes.." % dt.now()
                time.sleep(5*60)
...
counter = 0
for page in intesa.get_favorites():
    counter += 1
    print "[LOG %s ] Page: %s With %s Elements" % (dt.now(), counter, len(page))
    for tweet in page:
        db.favorites.insert(tweet._json)
