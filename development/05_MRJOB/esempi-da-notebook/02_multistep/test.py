...
class MRMostFreqWord(MRJob):
    ...
    def mapper_wordcount(self, _, line): #Questo e' il primo mapper
        words = WORD_REGEXP.findall(line)
        # Conto le parole di lunghezza maggiore a 3
        for w in words:
            if len(w)>3: #elimina le parole piu' corte
                yield w.lower(), 1

    def reducer_wordcount(self, word, counts): #questo e' il primo reduce
    ...
    def mapper_freq(self, word, total):
        if total > 1:  # Valuto solo le parole che compaiono piu di una volta
            yield total, word  # Le raggruppo per numero di occorrenze -> inverte le tuple per usare il max sulla lista di elementi
    ...
    def reducer_most(self, key, freqs):
        #print key
        #print freqs
